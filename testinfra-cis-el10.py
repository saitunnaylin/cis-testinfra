...
Full automated-control coverage for all TOC (Automated) items.

Manual CIS items are not included. GDM hardening tests skip when gdm is absent.
Journal-remote vs rsyslog path tests skip the inactive logging strategy.
"""
...
def _pkg_installed(host, pkg):
    """Check package install via rpm (avoid host.package → systeminfo on broken uname)."""
    return host.run("rpm -q %s 2>/dev/null", pkg).rc == 0
...
def _gdm_installed(host):
    return _pkg_installed(host, 'gdm')
...
def test_1_1_2_1_1_separate(host):
    # CIS RHEL 10: tmpfs or separate partition
    _findmnt(host, '/tmp')
...
def test_1_2_1_2_gpgcheck(host):
    result = host.run(
        "grep -E '^[[:space:]]*gpgcheck[[:space:]]*=[[:space:]]*1' "
        "/etc/dnf/dnf.conf /etc/yum.conf 2>/dev/null"
    )
    assert result.rc == 0, "gpgcheck=1 not found in /etc/dnf/dnf.conf or /etc/yum.conf"

def test_1_3_1_1_selinux_installed(host):
    _assert_pkg_installed(host, "libselinux")

def test_1_3_1_2_selinux_bootloader(host):
    result = host.run("grep -E '(selinux=0|enforcing=0)' /etc/default/grub /boot/grub2/grubenv /boot/efi/EFI/*/grubenv 2>/dev/null")
    assert result.stdout.strip() == ""

def test_1_3_1_3_selinux_policy(host):
    result = host.run("grep -E '^[[:space:]]*SELINUXTYPE[[:space:]]*=[[:space:]]*(targeted|mls)' /etc/selinux/config")
    assert result.rc == 0

def test_1_3_1_4_selinux_not_disabled(host):
    assert host.run("getenforce").stdout.strip() in ("Enforcing", "Permissive")

def test_1_3_1_5_selinux_enforcing(host):
    assert host.run("getenforce").stdout.strip() == "Enforcing"

def test_1_3_1_7_mcstrans(host):
    _package_not_installed(host, "mcstrans")

def test_1_3_1_8_setroubleshoot(host):
    _package_not_installed(host, "setroubleshoot")
...
def test_1_8_1_gdm_removed(host):
    _package_not_installed(host, "gdm")
...
def test_2_1_20_xorg(host):
    assert host.run("rpm -qa 'xorg-x11-server-*'").stdout.strip() == ""
...
def test_2_2_2_pkg(host):
    _package_not_installed(host, "openldap-clients")

def test_2_2_3_pkg(host):
    _package_not_installed(host, "ypbind")
...
def test_2_3_2_chrony_configured(host):
    result = host.run("grep -E '^(server|pool)[[:space:]]' /etc/chrony.conf")
    assert result.rc == 0

def test_2_3_3_chrony_user(host):
    unit = host.run("systemctl show chronyd -p User --value")
    assert unit.stdout.strip() != "root"

def test_2_4_1_1_crond(host):
    _assert_svc_enabled_running(host, "crond")
...
def test_4_1_1_nftables_pkg(host):
    _assert_pkg_installed(host, "nftables")

def test_4_1_2_single_fw(host):
    fw = _svc_active(host, "firewalld")
    nf = _svc_active(host, "nftables")
    assert fw or nf, "neither firewalld nor nftables is running"
    assert not (fw and nf), "both firewalld and nftables are running; only one firewall should be active"

def test_4_2_2_firewalld_lo(host):
    if not _svc_active(host, "firewalld"):
        pytest.skip("firewalld not active")
    rules = host.run("nft list ruleset 2>/dev/null")
    lo = host.run("firewall-cmd --zone=trusted --query-interface=lo 2>/dev/null")
    assert ("lo" in rules.stdout) or lo.rc == 0

def test_4_3_1_nft_chains(host):
    if _svc_active(host, "firewalld") and not _svc_active(host, "nftables"):
        pytest.skip("firewalld in use")
    out = host.run("nft list ruleset").stdout.lower()
    assert "input" in out and "forward" in out and "output" in out

def test_4_3_3_nft_deny(host):
    if _svc_active(host, "firewalld") and not _svc_active(host, "nftables"):
        pytest.skip("firewalld in use")
    out = host.run("nft list ruleset").stdout.lower()
    assert "policy drop" in out or "policy reject" in out

def test_4_3_4_nft_lo(host):
    if _svc_active(host, "firewalld") and not _svc_active(host, "nftables"):
        pytest.skip("firewalld in use")
    assert "lo" in host.run("nft list ruleset").stdout
...
def test_5_3_1_1_pam(host):
    _assert_pkg_installed(host, "pam")

def test_5_3_1_2_authselect(host):
    _assert_pkg_installed(host, "authselect")

def test_5_3_1_3_libpwquality(host):
    _assert_pkg_installed(host, "libpwquality")
...
def test_6_3_1_1_audit_pkgs(host):
    _assert_pkg_installed(host, "audit", "audit-libs")

def test_6_3_1_2_audit_cmdline(host):
    result = _cmd(
        host,
        "grep -E '(^|[[:space:]])audit=1([[:space:]]|$)' "
        "/etc/default/grub /boot/grub2/grubenv /boot/efi/EFI/*/grubenv "
